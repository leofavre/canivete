...
const _domElementsToArray = (arg, allowDocumentAndWindow = false) => {
  ...
  const isDom = allowDocumentAndWindow ?
    _isElementOrDocumentOrWindow :
    _isElement;

  const errorMsg = allowDocumentAndWindow ?
    'One or more HTMLElements are expected as the first parameter.' :
    'One or more HTMLElements, including document or window, ' +
    'are expected as the first parameter.';
  ...
  if (isDom(arg)) {
    ...
    arg != null &&
    (typeof arg.length === 'number' || typeof arg.size === 'number')
  ) {
    result = Array.from(arg);
  }

  _throwErrorIf(
    !Array.isArray(result) || result.some(item => !isDom(item)),
    errorMsg,
  );
