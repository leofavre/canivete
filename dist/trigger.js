 ...
 */
const trigger = (
  domEl, evtName, bubbles = false,
  cancelable = false, detail = undefined,
) => {
  _throwErrorIf(
    !_isElementOrDocumentOrWindow(domEl),
    'An HTMLElement, including document or window, ' +
    'is expected as first parameter.',
  );

  _throwErrorIf(
    !_isString(evtName),
    'A string is expected as second parameter.',
  );
  ...
  if (window.CustomEvent != null) {
    ...
    evt = new window.CustomEvent(evtName, {
      bubbles,
      cancelable,
      detail,
    });
